...
pre_export_rootfs_final_image() {
	...
	cp -r $SRC/userpatches/rootfs_overlay_llm/* ${MOUNT}/

	rm -rf ${MOUNT}/initrd.img ${MOUNT}/initrd.img.old ${MOUNT}/vmlinuz ${MOUNT}/vmlinuz.old
	pushd ${MOUNT}/usr/lib/modules
	ls
	cp -r 4.19.125/* 4.19.125-${BRANCH}-${LINUXFAMILY}/
	rm -rf 4.19.125
	ln -s 4.19.125-${BRANCH}-${LINUXFAMILY} 4.19.125
	popd
	rm -rf ${MOUNT}/var/cache/man/*

	echo 'tmpfs /tmp tmpfs defaults,nosuid 0 0' > ${MOUNT}/etc/fstab
	rm ${MOUNT}/usr/lib/udev/rules.d/80-net-setup-link.rules
	rm ${MOUNT}/root/.not_logged_in_yet

	rm ${MOUNT}/etc/systemd/system/basic.target.wants/*
	
	popd
}


if [ "$1" == "host_build" ] ; then
	return
fi
...
Chroot_Main() {
	...
	# apt-get install ifplugd -y
	apt-get install avahi-daemon -y
	apt-get install evtest -y
	apt install python3-pip libgl1 -y

	sed -i '/TimeoutStartSec=/s/.*/TimeoutStartSec=10sec/' ${MOUNT}/usr/lib/systemd/system/networking.service
